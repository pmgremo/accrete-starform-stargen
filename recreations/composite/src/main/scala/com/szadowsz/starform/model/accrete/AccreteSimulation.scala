...
abstract class AccreteSimulation[S <: Star, R <: SimulationStats[R], P <: Planetismal, X <: AbstractStarSystem[S, R, P]](
  ...
  /**
    ...
    */
  protected val logger: Logger = System.getLogger(getClass.getName)
    ...
    while (isDustAvailable(INNERMOST_PLANET, OUTERMOST_PLANET)) {
      ...
      if (isDustAvailable(proto.innerBandLimit, proto.outerBandLimit)) {
        ...
        if (proto.mass > PROTOPLANET_MASS) {
          ...
          logger.log(DEBUG, "Injecting protoplanet at %s AU.", proto.axis)
        }
      } else {
        logger.log(DEBUG, "Injection of protoplanet at %s AU failed due to no available dust.", proto.axis)
      }
    }
  }
