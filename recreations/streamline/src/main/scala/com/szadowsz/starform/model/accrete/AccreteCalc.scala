...
/**
  ...
  */
case class AccreteCalc(star: Star, pCalc: PlanetesimalCalc, aConst: AccreteConstants) {
  ...
  /**
    ...
    */
  def outerDustLimit(mass: Double): Double = 200 * pow(mass, 1.0 / 3.0)
  ...
  /**
    ...
    */
  def dustDensity(radius: Double): Double = (aConst.DUST_DENSITY_COEFF * sqrt(star.mass)) * exp(-aConst.ALPHA * pow(radius, 1.0 / aConst.N))
  ...
  /**
    ...
    */
  def dustAndGasDensity(dustDensity: Double, criticalMass: Double, mass: Double): Double = aConst.K * dustDensity / (1.0 + sqrt(criticalMass / mass) * (aConst.K - 1.0))
  ...
  def bandVolume(mass: Double, axis: Double, ecc: Double,
    ...
    if outside < 0.0 then outside = 0.0

    var inside = iEdge - iSweep
    if inside < 0.0 then inside = 0.0

    val width = bandwidth - outside - inside

    val term1 = 4.0 * PI * axis * axis
